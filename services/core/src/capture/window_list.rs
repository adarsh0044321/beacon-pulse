...
use windows::Win32::{
        ...
        UI::WindowsAndMessaging::{
            ...
            IsIconic, IsWindowVisible, GetWindow, GW_OWNER, GetWindowLongW, GWL_STYLE,
            WS_CHILD,
        },
    };
...
unsafe extern "system" fn enum_window_callback(hwnd: HWND, lparam: LPARAM) -> BOOL {
    ...
    if style & WS_CHILD.0 != 0 { return BOOL(1); }
    ...
    if let Ok(owner_hwnd) = owner {
        if !owner_hwnd.is_invalid() { return BOOL(1); }
    }
    ...
    let skip_titles = ["Program Manager", "Windows Input Experience",
                        "Settings", "Task View", "Windows Shell Experience Host"];
    if skip_titles.iter().any(|s| title.starts_with(s)) { return BOOL(1); }
    ...
    if width < 50 || height < 50 { return BOOL(1); }
