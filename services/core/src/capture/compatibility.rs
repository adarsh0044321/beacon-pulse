...
use windows::Win32::{
    Foundation::HWND,
    UI::WindowsAndMessaging::{GetWindowLongW, GWL_EXSTYLE, WS_EX_NOREDIRECTIONBITMAP},
};
...
pub fn detect_app_kind(info: &WindowInfo) -> AppKind {
    ...
    let game_hints = [
        "unity", "unreal", "dx11", "dx12", "d3d", "opengl", "vulkan", "steam", "epic", "game",
    ];
    if game_hints
        .iter()
        .any(|h| proc.contains(h) || title.contains(h))
    {
...
pub fn suspends_render_when_minimized(kind: &AppKind) -> bool {
    matches!(
        kind,
        AppKind::Chromium | AppKind::DirectX | AppKind::UWP | AppKind::Vulkan
    )
}
...
#[cfg(not(windows))]
fn is_uwp_window(_hwnd: isize) -> bool {
    false
}
