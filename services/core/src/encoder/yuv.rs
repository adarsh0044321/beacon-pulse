...
pub fn bgra_to_nv12(bgra: &[u8], width: u32, height: u32) -> Vec<u8> {
    ...
    for row in 0..h {
        ...
        while col + 1 < w {
            ...
            dst_row[col + 1] = (((66 * r1 + 129 * g1 + 25 * b1 + 128) >> 8) + 16).clamp(16, 235) as u8;
            col += 2;
        }
...
mod tests {
    ...
    fn test_white_pixel_yuv420p() {
        ...
        let bgra = vec![255u8, 255, 255, 255,  // pixel (0,0)
                        255u8, 255, 255, 255,  // pixel (1,0)
                        255u8, 255, 255, 255,  // pixel (0,1)
                        255u8, 255, 255, 255]; // pixel (1,1)
        let (y, _u, _v) = bgra_to_yuv420p(&bgra, 2, 2);
        assert!((y[0] as i32 - 235).abs() <= 2, "white Y should be ~235, got {}", y[0]);
    }
    ...
    fn test_nv12_size() {
        ...
        assert_eq!(nv12.len(), (w * h + w * h / 2) as usize, "NV12 buffer size mismatch");
    }
    ...
    fn test_nv12_white_pixel() {
        ...
        assert!((nv12[0] as i32 - 235).abs() <= 2, "white Y should be ~235, got {}", nv12[0]);
        // U byte at offset 4 should be ~128 (neutral chroma)
        assert!((nv12[4] as i32 - 128).abs() <= 3, "white U should be ~128, got {}", nv12[4]);
    }
